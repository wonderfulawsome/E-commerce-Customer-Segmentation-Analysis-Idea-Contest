...
# Matplotlib 마이너스 기호 깨짐 방지 (폰트 설정은 제거하여 시스템 기본 영문 폰트 사용)
...
if menu == "1. 대시보드 개요":
    ...
    with col_chart1:
        ...
        # Plotly는 한글이 잘 나오므로 그대로 사용
        fig1 = px.line(monthly, x='월', y='지불금액', markers=True, title="월별 매출 (Monthly Revenue)")
        st.plotly_chart(fig1, use_container_width=True)

    with col_chart2:
        st.subheader("지역별 고객 분포")
        region_cnt = df_filtered.groupby('고객지역')['고객ID'].nunique().reset_index()
        fig2 = px.pie(region_cnt, values='고객ID', names='고객지역', hole=0.4, title="지역별 고객 (Region)")
...
# --- Page 3: 리텐션 (Matplotlib 한글 깨짐 방지를 위해 축 이름만 영어로 변경) ---
elif menu == "3. 리텐션 & 코호트":
    ...
    seg_eng_map = {
        ...
        "VIP고객": "VIP",
        "충성고객": "Loyal",
        "잠재충성고객": "Potential Loyal",
        "놓치면안될고객": "Can't Lose",
        "최근신규방문고객": "New Customers",
        "이탈우려고객": "At Risk",
        "기타": "Others"
    }
...
# --- Page 4: 연관 분석 ---
    ...
    # UI 한국어
    target_seg = st.selectbox("분석 대상 세그먼트", ["전체", "잠재충성고객", "이탈우려고객"])
        ...
        with st.spinner("Apriori 알고리즘 실행 중..."):
            ...
            if frequent.empty:
                ...
                fig = px.scatter(rules, x="support", y="confidence", size="lift", color="lift",
                                 ...
                                 labels={'support': '지지도', 'confidence': '신뢰도'})
